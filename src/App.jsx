...
const ParticleBackground = () => {
  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
      <div className="absolute inset-0 bg-glow-1"></div>
      <div className="absolute inset-0 bg-glow-2"></div>
      {particles.map((p) => (
        <div
          key={p.id}
          className="particle"
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-body-md text-on-surface bg-background relative overflow-hidden pb-20 md:pb-0">
      <ParticleBackground />

      {/* Top Navigation Header */}
      <header className="fixed top-0 w-full z-50 bg-surface/30 backdrop-blur-xl border-b border-white/10 flex justify-between items-center px-margin-mobile md:px-margin-desktop h-16 shadow-[0_0_20px_rgba(76,215,246,0.1)]">
        <div className="flex items-center gap-base">
          <span className="material-symbols-outlined text-secondary" style={{ fontVariationSettings: "'FILL' 1" }}>casino</span>
          <h1 className="font-headline-lg-mobile text-headline-lg-mobile font-bold text-secondary tracking-tighter">
            Ruleta de estudio
          </h1>
        </div>
        <div className="flex items-center gap-md">
          {view !== 'config' && (
            <button
              ...
              className="hidden md:flex items-center gap-1.5 font-label-caps text-label-caps text-outline hover:text-secondary transition-colors uppercase tracking-widest cursor-pointer"
            >
              <span className="material-symbols-outlined text-sm">settings</span>
              <span>Configuración</span>
            </button>
          )}
          <span className="material-symbols-outlined text-outline hover:text-secondary transition-colors active:scale-95 duration-200 cursor-pointer">
            account_circle
          </span>
        </div>
      </header>
      ...
      {/* Main Content Canvas */}
      <main className="relative z-10 pt-20 pb-20 flex-grow flex flex-col justify-center max-w-7xl mx-auto w-full">
        {view === 'config' && (
          <ConfigurationScreen
            players={players}
      ...
      <nav className="md:hidden fixed bottom-0 w-full z-50 bg-surface-container/20 backdrop-blur-2xl border-t border-white/5 flex justify-around items-center h-20 px-4 pb-2 shadow-[0_-4px_20px_rgba(208,188,255,0.15)]">
        <button
          onClick={handleGoToConfigReset}
          className={`flex flex-col items-center justify-center rounded-xl px-4 py-1 transition-all duration-150 active:scale-90 cursor-pointer border-none bg-transparent ${
            view === 'config'
              ? 'text-primary bg-primary/10 shadow-[0_0_15px_rgba(208,188,255,0.3)]'
              : 'text-outline-variant hover:bg-white/5'
          }`}
        >
          <span className="material-symbols-outlined" style={{ fontVariationSettings: view === 'config' ? "'FILL' 1" : "'FILL' 0" }}>home</span>
          <span className="font-label-caps text-label-caps">Inicio</span>
        </button>

        <button
          onClick={() => {
            if (players.length > 0 && questions.length > 0) setView('game');
          }}
          disabled={players.length === 0 || questions.length === 0}
          className={`flex flex-col items-center justify-center rounded-xl px-4 py-1 transition-all duration-150 active:scale-90 cursor-pointer border-none bg-transparent disabled:opacity-30 disabled:cursor-not-allowed ${
            view === 'game'
              ? 'text-primary bg-primary/10 shadow-[0_0_15px_rgba(208,188,255,0.3)]'
              : 'text-outline-variant hover:bg-white/5'
          }`}
        >
          <span className="material-symbols-outlined" style={{ fontVariationSettings: view === 'game' ? "'FILL' 1" : "'FILL' 0" }}>casino</span>
          <span className="font-label-caps text-label-caps">Juego</span>
        </button>

        <button
          onClick={() => {
            if (players.length > 0) setView('podium');
          }}
        ...
        >
          <span className="material-symbols-outlined" style={{ fontVariationSettings: view === 'podium' ? "'FILL' 1" : "'FILL' 0" }}>emoji_events</span>
          <span className="font-label-caps text-label-caps">Podio</span>
        </button>
      </nav>
        ...
        <AnimatePresence>
          {notification && (
            <motion.div
              ...
              className={`w-full pointer-events-auto p-4 rounded-xl shadow-2xl border flex items-start gap-3 backdrop-blur-md ${
            ...
            >
              {notification.type === 'error' && <span className="material-symbols-outlined text-rose-400 mt-0.5">error</span>}
              {notification.type === 'success' && <span className="material-symbols-outlined text-emerald-400 mt-0.5">check_circle</span>}
              {notification.type !== 'error' && notification.type !== 'success' && <span className="material-symbols-outlined text-violet-400 mt-0.5">info</span>}
        ...
        {showConfirmReset && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="glass-panel p-6 rounded-xl max-w-md w-full border border-white/10 shadow-2xl text-left"
            >
              <div className="flex items-center gap-3 text-amber-500 mb-4">
                <span className="material-symbols-outlined text-3xl">warning</span>
                <h3 className="text-xl font-bold text-on-surface">¿Restablecer valores?</h3>
              </div>
              ...
              <div className="flex gap-3 justify-end">
                <button
                  ...
                  className="px-4 py-2 rounded-lg border border-white/10 hover:bg-white/5 text-outline hover:text-on-surface transition-all font-semibold text-sm cursor-pointer"
                >
                  Cancelar
                </button>
                <button
                  onClick={confirmResetToDefault}
                  className="px-5 py-2 rounded-lg bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 hover:to-red-500 text-white font-bold text-sm shadow-lg hover:shadow-rose-600/20 transition-all cursor-pointer active:scale-95"
                >
                  Sí, Restablecer
                </button>
              </div>
